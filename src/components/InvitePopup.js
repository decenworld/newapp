...
const InvitePopup = ({ onClose }) => {
  ...
  React.useEffect(() => {
    if (isTelegramAvailable && telegramWebApp) {
      ...
      setReferralLink(`https://t.me/cookieclickergame?start=${newReferralCode}`);
    } else {
      // Generate a dummy referral link for local mode
      setReferralLink(`https://t.me/cookieclickergame?start=${Math.random().toString(36).substring(2, 8)}`);
    }
